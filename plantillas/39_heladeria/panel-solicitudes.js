...
function enviarWhatsApp(telefono, folio) {
    ...
    const mensaje = `Hola, te contactamos de Plantilla Abarrotes respecto a tu solicitud ${folio}.`;
...
function renderCotizacionItems() {
    ...
    cotizacionItems.forEach((it, idx) => {
        ...
        row.innerHTML = `
            <input data-field="concepto" data-idx="${idx}" class="col-span-6 rounded bg-[#0f0f0f] border border-[{{COLOR_PRIMARY}}]/30 px-2 py-2 text-sm" placeholder="Concepto" value="${escapeHtml(it.concepto || '')}">
            <input data-field="cantidad" data-idx="${idx}" type="number" min="1" step="1" class="col-span-2 rounded bg-[#0f0f0f] border border-[{{COLOR_PRIMARY}}]/30 px-2 py-2 text-sm text-right" value="${Number(it.cantidad || 1)}">
            <input data-field="precio" data-idx="${idx}" type="number" min="0" step="0.01" class="col-span-3 rounded bg-[#0f0f0f] border border-[{{COLOR_PRIMARY}}]/30 px-2 py-2 text-sm text-right" value="${Number(it.precio || 0)}">
    ...
    const html = `
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .header { background: linear-gradient(135deg, #0F4C81 0%, {{COLOR_PRIMARY}} 100%); color: #fff; padding: 28px 34px; display: flex; justify-content: space-between; align-items: center; }
                .header h1 { font-size: 30px; font-weight: 800; letter-spacing: 1px; }
                .header h1 span { color: {{COLOR_ACCENT}}; }
                ...
                .card h3 { font-size: 15px; color: {{COLOR_PRIMARY}}; margin-bottom: 10px; border-bottom: 2px solid {{COLOR_ACCENT}}; padding-bottom: 6px; }
                ...
                .desc { margin-top: 18px; background: #fff7ed; border-left: 6px solid {{COLOR_ACCENT}}; padding: 16px; border-radius: 12px; }
        ...
        <body>
            <div class="container">
                ...
                <div class="footer">Plantilla Abarrotes · {{CITY}} · {{WHATSAPP_HUMAN}}</div>
            </div>
...
function render(solicitudes) {
    ...
    solicitudes.forEach(s => {
        ...
        card.className = 'bg-[#2B2B2B] border border-[{{COLOR_PRIMARY}}]/40 rounded-xl p-4';
        card.innerHTML = `
            <div class="flex justify-between items-start gap-2">
                <div>
                    <h3 class="font-bold text-[{{COLOR_PRIMARY}}]">${escapeHtml(s.FOLIO_COTIZACION || 'Sin folio')}</h3>
            ...
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <button class="btn-cotizar bg-[{{COLOR_PRIMARY}}] hover:bg-blue-500 text-white rounded-lg py-2 text-sm font-semibold" data-folio="${escapeHtml(s.FOLIO_COTIZACION)}">

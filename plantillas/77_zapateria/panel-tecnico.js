        ...
        function actualizarHoraActualizacion() {
            document.getElementById('last-update').innerHTML = `<i class="fa-regular fa-clock mr-1 text-[{{COLOR_PRIMARY}}]"></i> ${new Date().toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', second:'2-digit' })}`;
        }
            ...
            equiposFiltrados.forEach(eq => {
                ...
                card.innerHTML = `
                    ...
                    <div class="flex flex-wrap gap-2 items-center justify-between mt-3">
                        ...
                        <span class="text-xs text-[{{COLOR_PRIMARY}}] flex items-center gap-1"><i class="fa-regular fa-eye"></i> Ver detalles</span>
                    </div>
                `;
        ...
        function mostrarSeccion(tabId) {
            document.querySelectorAll('[data-tab]').forEach(btn => {
                ...
                btn.classList.toggle('text-[{{COLOR_PRIMARY}}]', active);
                btn.classList.toggle('border-b-2', active);
                btn.classList.toggle('border-[{{COLOR_PRIMARY}}]', active);
        ...
        function construirWaUrl(telefono, folio) {
            ...
            const mensaje = `Hola, te escribimos de Plantilla Abarrotes sobre tu equipo con folio ${folio}.`;
        ...
        function enviarWhatsAppCliente() {
            ...
            const limpio = String(telefono).replace(/\D/g, '');
            const destino = limpio.length === 10 ? `52${limpio}` : limpio;
            const mensaje = `Hola, te escribimos de Plantilla Abarrotes para informarte que tu equipo con folio ${folio} se encuentra en estado: ${estado}.`;
        ...
        function descargarFichaPDF() {
            ...
            const html = `
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Ficha ${e.FOLIO || 'PLANTILLA ABARROTES'}</title>
                    ...
                    <style>
                        *{margin:0;padding:0;box-sizing:border-box}
                        ...
                        .header{background:linear-gradient(135deg,#0F4C81 0%,{{COLOR_PRIMARY}} 100%);color:#fff;padding:30px 35px;display:flex;justify-content:space-between;align-items:center}
                        .header h1{font-size:30px;font-weight:800;letter-spacing:1px}.header h1 span{color:{{COLOR_ACCENT}}}
                        ...
                        .card h3{font-size:16px;color:{{COLOR_PRIMARY}};margin-bottom:12px;border-bottom:2px solid {{COLOR_ACCENT}};padding-bottom:6px}
                        ...
                        .notas{background:#fff7ed;border-left:6px solid {{COLOR_ACCENT}};padding:16px;border-radius:12px;margin-top:18px}
        ...
        function renderizarGaleriaSeguimiento() {
            ...
            seguimientoFotosBase64.forEach((src, idx) => {
                ...
                item.className = 'relative rounded-lg overflow-hidden border border-[{{COLOR_PRIMARY}}] bg-[#1E1E1E]';
                item.innerHTML = `
        ...
        function mostrarToast(mensaje, tipo = 'success') {
            ...
            toast.style.borderLeftColor = tipo === 'error' ? '#ef4444' : '{{COLOR_ACCENT}}';
            setTimeout(() => {

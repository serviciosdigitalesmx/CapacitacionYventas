/**
 * {{BRAND_UPPER}} CLOUD - Backend con Contraseñas
...
// ==========================================
const CONFIG = {
  SHEET_NAME: '{{BRAND_UPPER}}_DATABASE',
  API_VERSION: '2.5.0',
  SCRIPT_PROP_KEYS: {
    TECNICO: '{{BRAND_UPPER}}_PASSWORD_TECNICO',
    OPERATIVO: '{{BRAND_UPPER}}_PASSWORD_OPERATIVO',
    FOLIO_EQUIPO_SEQ: '{{BRAND_UPPER}}_FOLIO_EQUIPO_SEQ',
    FOLIO_COTIZACION_SEQ: '{{BRAND_UPPER}}_FOLIO_COTIZACION_SEQ',
    FOLIO_COTIZACION_MANUAL_SEQ: '{{BRAND_UPPER}}_FOLIO_COTIZACION_MANUAL_SEQ',
    DRIVE_FOLDER_ID: '{{BRAND_UPPER}}_DRIVE_FOLDER_ID'
  },
  LIMITS: {
    MAX_PAGE_SIZE: 2000,
...
function obtenerPasswords() {
  ...
  if (!tecnico || !operativo) {
    throw new Error('Configura {{BRAND_UPPER}}_PASSWORD_TECNICO y {{BRAND_UPPER}}_PASSWORD_OPERATIVO en Script Properties');
  }
...
function getConfig() {
  ...
  try {
    ...
    return {
      b: { name: '{{BRAND}}', slogan: 'Servicio profesional' },

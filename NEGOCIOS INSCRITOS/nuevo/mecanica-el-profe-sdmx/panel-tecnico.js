        ...
        function actualizarHoraActualizacion() {
            document.getElementById('last-update').innerHTML = `<i class="fa-regular fa-clock mr-1 text-[#9d231b]"></i> ${new Date().toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit', second:'2-digit' })}`;
        }
            ...
            equiposFiltrados.forEach(eq => {
                ...
                card.innerHTML = `
                    ...
                    <div class="flex flex-wrap gap-2 items-center justify-between mt-3">
                        ...
                        <span class="text-xs text-[#9d231b] flex items-center gap-1"><i class="fa-regular fa-eye"></i> Ver detalles</span>
                    </div>
                `;
        ...
        function mostrarSeccion(tabId) {
            document.querySelectorAll('[data-tab]').forEach(btn => {
                ...
                btn.classList.toggle('text-[#9d231b]', active);
                btn.classList.toggle('border-b-2', active);
                btn.classList.toggle('border-[#9d231b]', active);
        ...
        function construirWaUrl(telefono, folio) {
            ...
            const mensaje = `Hola, te escribimos de Plantilla Tecnico sobre tu equipo con folio ${folio}.`;
        ...
        function enviarWhatsAppCliente() {
            ...
            const limpio = String(telefono).replace(/\D/g, '');
            const destino = limpio.length === 10 ? `52${limpio}` : limpio;
            const mensaje = `Hola, te escribimos de Plantilla Tecnico para informarte que tu equipo con folio ${folio} se encuentra en estado: ${estado}.`;
        ...
        function descargarFichaPDF() {
            ...
            const html = `
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>Ficha ${e.FOLIO || 'PLANTILLA TECNICO'}</title>
                    ...
                    <style>
                        *{margin:0;padding:0;box-sizing:border-box}
                        ...
                        .header{background:linear-gradient(135deg,#0F4C81 0%,#9d231b 100%);color:#fff;padding:30px 35px;display:flex;justify-content:space-between;align-items:center}
                        .header h1{font-size:30px;font-weight:800;letter-spacing:1px}.header h1 span{color:#933e1a}
                        ...
                        .card h3{font-size:16px;color:#9d231b;margin-bottom:12px;border-bottom:2px solid #933e1a;padding-bottom:6px}
                        ...
                        .notas{background:#fff7ed;border-left:6px solid #933e1a;padding:16px;border-radius:12px;margin-top:18px}
        ...
        function renderizarGaleriaSeguimiento() {
            ...
            seguimientoFotosBase64.forEach((src, idx) => {
                ...
                item.className = 'relative rounded-lg overflow-hidden border border-[#9d231b] bg-[#1E1E1E]';
                item.innerHTML = `
        ...
        function mostrarToast(mensaje, tipo = 'success') {
            ...
            toast.style.borderLeftColor = tipo === 'error' ? '#ef4444' : '#933e1a';
            setTimeout(() => {
